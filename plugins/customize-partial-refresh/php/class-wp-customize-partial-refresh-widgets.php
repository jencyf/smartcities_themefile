...
class WP_Customize_Partial_Refresh_Widgets {
	...
	function customize_preview_enqueue_deps() {
		...
		if ( ! empty( $_SERVER['REQUEST_URI'] ) ) { // WPCS: input var ok
			$exports['requestUri'] = esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ); // WPCS: input var ok
		}
		$wp_scripts->add_data(
	...
	function render_widget() {
		...
		$generic_error = __( 'An error has occurred. Please reload the page and try again.', 'optimizer' );
		try {
			...
			if ( empty( $_POST[ self::RENDER_WIDGET_NONCE_POST_KEY ] ) ) { // wpcs: input var okay
				throw new WP_Customize_Partial_Refresh_Exception( __( 'Missing nonce param', 'optimizer' ) );
			}
			if ( ! check_ajax_referer( self::RENDER_WIDGET_AJAX_ACTION, self::RENDER_WIDGET_NONCE_POST_KEY, false ) ) {
				throw new WP_Customize_Partial_Refresh_Exception( __( 'Nonce check failed. Reload and try again?', 'optimizer' ) );
			}
			if ( ! current_user_can( 'edit_theme_options' ) ) {
				throw new WP_Customize_Partial_Refresh_Exception( __( 'Current user cannot!', 'optimizer' ) );
			}
			if ( empty( $_POST['widget_id'] ) ) { // wpcs: input var okay
				throw new WP_Customize_Partial_Refresh_Exception( __( 'Missing widget_id param', 'optimizer' ) );
			}
			...
			if ( ! array_key_exists( $widget_id, $wp_registered_widgets ) ) {
				throw new WP_Customize_Partial_Refresh_Exception( __( 'Unable to find registered widget', 'optimizer' ) );
			}
			$widget = $wp_registered_widgets[ $widget_id ];

			if ( empty( $_POST['sidebar_id'] ) ) { // wpcs: input var okay
				throw new WP_Customize_Partial_Refresh_Exception( __( 'Missing sidebar_id param', 'optimizer' ) );
			}
			...
			if ( empty( $sidebar ) ) {
				throw new WP_Customize_Partial_Refresh_Exception( sprintf( __( 'Attempted to render widget %1$s onto unknown sidebar %2$s.', 'optimizer' ), $widget_id, $sidebar_id ) );
			}
			...
			if ( ! is_array( $callback ) || ! ( $callback[0] instanceof WP_Widget ) ) {
				throw new WP_Customize_Partial_Refresh_Exception( __( 'Only Widgets 2.0 are supported. Old single widgets are not.', 'optimizer' ) );
			}

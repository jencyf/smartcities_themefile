...
require(get_template_directory() . '/customizer/includes/control-gallery.php');
...
require(get_template_directory() . '/customizer/includes/control-sidebars.php');
...
//========================= ADD SECTIONS==============================
        ...
        $wp_customize->add_section( 'miscseo_section', array(
            ...
            'priority'    => 10,
            'panel'       => 'misc_panel',
			'description' => __( 'You won\'t see live changes in the preview panel for the changes you make in this section. These options work in the background.', 'optimizer' ),
        ) );
        ...
        $wp_customize->add_section( 'customfont_section', array(
            'title'       => __( 'Custom Font', 'optimizer' ),
            'priority'    => 10,
            'panel'       => 'misc_panel',
        ) );
        ...
        $wp_customize->add_section( 'video_section', array(
            'title'       => __( 'Video Instructions', 'optimizer' ),
            'priority'    => 10,
            'panel'       => 'help_panel',
        ) );
...
require(get_template_directory() . '/customizer/controls/settings-plugins.php');
...
function optimizer_live_preview()
{
	...
	wp_enqueue_style( 'optimizer_colpickcss', get_template_directory_uri() . '/assets/css/colpick.css' );
	//wp_enqueue_script( 'jquery-ui-core' );
	//wp_enqueue_script( 'jquery-ui-resizable' );
	//wp_enqueue_style('jquery-ui-css', 'https://ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/themes/smoothness/jquery-ui.css', false, false);
	...
	if(!empty($optimizer['inline_editor'])){
		wp_enqueue_script(  'optimizer-inlineditor', get_template_directory_uri().'/customizer/assets/inlineditor.js',array( 'jquery','customize-preview' ),true);
	}
	wp_enqueue_style( 'optimizer_livecss', get_template_directory_uri() . '/assets/css/live.css' );
}
...
function enqueue_customizer_scripts(){
	...
	//Custom Font Exist Show Differnt Message
	global $optimizer;
	$fontonmsg ='';
	if( $optimizer['custom_font_ttf'] && $optimizer['custom_font_eot'] && $optimizer['custom_font_woff'] ){
		...
		$fontonmsg = sprintf(__( 'Your Current Active Font is: %s You can select this front from Basic > Basic Fonts', 'optimizer' ), '<span class="current_cfont">'.$fontname.'</span>');
	}
	...
	wp_localize_script( 'optimizer-customizer-js', 'objectL10n', array(
		...
		'fontstep1' => sprintf( __( 'Instruction: Upload your custom font to %s and convert the font. You will get a zip file. Extract The zip file and upload the ttf, eot &amp; the woff file that you got from the zip file and click the "Activate" button.', 'optimizer' ), '<a target="_blank" href="http://transfonter.org/">transfonter</a>' ),
		'fontstep2' => __( 'Activate Font', 'optimizer' ),
		'fontstep3' => __( 'You can Insert 1 Custom Font', 'optimizer' ),
		'fontstep4' => __( 'Deactivate Font', 'optimizer' ),
		'fontstep5' => __( 'Currently Active Font (1/1)', 'optimizer' ),
		'customfontfocus' => admin_url('customize.php?autofocus[section]=customfont_section'),
		'fontonmsg' => $fontonmsg,
		...
		'getmapkey' => __( '<div class="custom_desc">If your Maps are not working you will have to <a href="https://optimizerwp.com/documentation/get-google-map-api-key/" target="_blank">Get a Map Api Key</a>.</div>', 'optimizer' ),
		'wp4_7' => $wp4_7,
